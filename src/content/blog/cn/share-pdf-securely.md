...
水印这层的逻辑不是"防止被看见"，而是**让被转发/被截图的文件能被追回责任**。MaiPDF 的动态水印会自动把**本次打开的记录编号**盖在每一页上——每次打开的编号都是独一份，对应到那一次访问。身份信息（IP、浏览器、验证过的邮箱或手机号）不印在页面上，而是存在编号指向的记录里，只有你能查。

实际效果：

- **截图可溯源** — 对方截图转到群里，水印跟着一起被截出去，一眼就能对到是谁。
- **每次打开水印都不同** — 同一份 PDF、同一个链接，张三打开是一个编号、李四打开是另一个；同一个人打开两次也是两个编号。
...
- **需要完全离线阅读**（对方没网、内网环境）—— 需要 MaiPDF 的 **App DRM** 工作流（在 [drm.maipdf.com](https://drm.maipdf.com/) 生成加密 `.maipdf`，U 盘分发，用 App 打开），本文不展开。
