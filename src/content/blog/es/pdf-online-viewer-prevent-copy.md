...
```mermaid
flowchart TD
    A[Usuario Abre Enlace PDF] --> B[Cargar Visor en Línea]
    B --> C[Habilitar Prevenir Copia]
    C --> D{Acción del Usuario}
    D -->|Intentar Copiar Texto| E[Bloquear Operación de Copia]
    D -->|Intentar Guardar con Clic Derecho| F[Desactivar Menú de Clic Derecho]
    D -->|Intentar Captura de Pantalla| G[No Se Puede Prevenir Completamente]
    D -->|Visualización Normal| H[Permitir Visualización]
    E --> I[Mostrar Mensaje de Advertencia]
    F --> I
    H --> J[Navegación Normal del Documento]
