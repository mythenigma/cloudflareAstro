...
| 站点 | 验证方式 | 机制 |
|---|---|---|
| maipdf.cn | 手机号白名单（**仅中国大陆号码，上限 50 个**）| 上传者指定若干手机号，仅名单内的号码收到 SMS 验证码后可打开；超过 50 个或境外号码不支持 |
| maipdf.com | 邮箱白名单 | 上传者指定邮箱列表，仅名单内邮箱收到验证邮件 |

- 国际版因覆盖地区多、不走 SMS，改用邮箱
- ❌ EN 博客**不要**写"SMS 验证"或"手机号白名单"
- ❌ CN 博客**不要**写"邮箱验证"作为核心验证方式
- ❌ CN 博客**不要**写"境外手机号"或"无人数上限"（两者都是假功能，实际上限 50 个大陆号）

### 阅读提醒（**CN 和 EN 不同**）

| 站点 | 提醒方式 | 现状 |
|---|---|---|
| maipdf.cn | 邮件/站内 | 送达率低，多数进垃圾箱，**实际基本不可用** —— CN 博客不要炒作这个功能；**`maipdf.cn` 没有 Telegram 机器人** |
| maipdf.com | **Telegram 机器人**实时提醒 | 用户绑定 bot 后收到访问通知。**调用量限制在 10,000 次以内** |

- ❌ CN 博客**绝对不要**写 Telegram —— `maipdf.cn` 根本没有这个功能，且中国读者大多数不认识 Telegram
...
| **Telegram（CN 博客）** | ❌ `maipdf.cn` 没有 Telegram，EN 独有 |
| 手机号白名单无上限 / 支持境外号（CN）| ❌ 实际上限 50 个中国大陆号码 |
