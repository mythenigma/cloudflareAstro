...
- 区块链存证、AI 异常行为检测、地理围栏
- 网页端的防截屏（浏览器原理上做不到；<strong>但 App 版可以</strong>——由操作系统直接拒绝截图，见 App DRM）
- 黑名单（只有白名单）
...
- **完全离线分发**——对方在内网/保密机房，链接打不开。离线分发走 **App DRM**：在 [drm.maipdf.com](https://drm.maipdf.com/) 把 PDF 打包成加密的 `.maipdf`，收件人用 MaiPDF App 打开，不依赖公网。（早年那套"下载加密 .html 包"的做法仍然能用，但已不推荐，新分享请走 App DRM。）
