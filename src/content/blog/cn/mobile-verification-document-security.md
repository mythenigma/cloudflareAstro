...
上传 PDF 之后（单文件 ≤ 80 MB），在文件设置面板里找到**手机号验证**开关，勾选后会出现号码输入框：
...
- **手机验证 + 动态水印**：水印在每一页盖上本次打开的编号。截图泄露时，拿编号去查访问记录，就能定位到验证过的那个手机号。页面上不会直接印出号码或 IP——这反而是好事，旁人瞄一眼学不到任何东西，你手上却有完整的追踪链。
