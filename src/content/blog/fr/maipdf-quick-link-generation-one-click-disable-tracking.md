...
```mermaid
flowchart TD
    Start([Début]) --> A[Télécharger le PDF du contrat]
    ...
    F -->|Oui| G[Analyser les points d'intérêt du client]
    F -->|Non| H[Rappeler au client]
    G --> I{Le contrat est-il signé?}
    H --> I
    I -->|Oui| J[Désactivation en un clic]
    I -->|Non| K[Garder le lien actif]
    J --> L[Protéger la confidentialité du contrat]
    L --> End([Fin])
    K --> D
```
