...
![Same email, same send button — but a link keeps working where an attachment would have bounced](/maipdf2026/show_off/shareandsend.png)

## Quick navigation

- [The six problems attachments can't solve](#the-six-problems-attachments-cant-solve)
- [Side-by-side comparison](#side-by-side-comparison)
- [What changes in the email itself](#what-changes-in-the-email-itself)
- [When attachments still make sense](#when-attachments-still-make-sense)
- [Email phrasing patterns that work](#email-phrasing-patterns-that-work)
- [Transition playbook](#transition-playbook)
...
> Hi Jamie,
>
> Please find our Q2 proposal here: [maipdf.com/…](https://maipdf.com)
>
...
![One link lives in the same email channel you already use](/maipdf2026/Result_of_qr_link.png)

## Transition playbook

Most teams that switch successfully do it in three steps, not one.

1. **Pilot with one document type for two weeks.** Pick the one that most often bounces or gets version confusion — usually proposals or internal drafts. Don't try to convert everything at once.
2. **Fix the email template.** Paste the link in place of the attachment in your most-used template. That's usually where 80% of the daily attachment volume lives.
3. **Measure and expand.** Check the access log on a few sends. When you see the signal ("I can tell whether they actually read it" is the usual reaction), expand to the next document type.
...
**Won't recipients be suspicious of a link instead of an attachment?**
In 2026, almost nobody is. Every big SaaS company (DocuSign, Notion, Google, Linear) sends link-only emails. What actually raises suspicion is a bare, uncontextualized URL — add one sentence of context and recipients read it like any other link.

**What if the recipient's corporate firewall blocks the domain?**
Uncommon but possible. In that case, send the attachment as a fallback. You can also brand-customize the sharing URL if that's an ongoing concern.

**Does the recipient need an account?**
No. Opening a MaiPDF link works in any modern browser without a sign-in or plugin on the reader's side.

**Is it free?**
Yes — core sharing is free. Some higher-usage features (bigger files, heavier controls) need an account.

**What if I accidentally sent the link to the wrong person?**
Revoke from the control panel — the link stops working immediately. You can also add email verification after the fact so only specific addresses can open it going forward, without changing the URL.

**Does this replace DocuSign / e-signature tools?**
No — MaiPDF distributes the PDF; e-signature tools collect signatures. They're complementary: many teams send the draft-for-review as a MaiPDF link, then move to e-signature for the final execution.
