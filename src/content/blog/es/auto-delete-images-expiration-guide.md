...
```mermaid
flowchart TD
    Start([Subir Imágenes]) --> A[Establecer Fecha de Expiración]
    A --> B[Imágenes Compartidas]
    B --> C[Imágenes Accesibles]
    C --> D{¿Se Alcanzó la Expiración?}
    D -->|No| C
    D -->|Sí| E[Enlace Auto Desactivado]
    E --> F[Imágenes Ya No Accesibles]
    F --> G[Eliminación con Un Clic Disponible]
    G --> End([Completo])
