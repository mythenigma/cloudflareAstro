...
- **多种分享方式**：拿到链接和二维码后，你可以通过任何渠道发出去
- **快速分享设置**：快速配置权限和安全选项
- **一次一个文件**：MaiPDF 不支持批量上传，也没有文件夹概念，每份 PDF 生成自己的链接

### 智能分享控制

- **有效期设置**：设置链接自动过期时间
- **访问限制**：限制打开次数（网页版按总次数计；设备数限制只有 App 版有）
- **视图模式**：SecureView（移除下载与打印入口）或开放模式；<strong>没有"编辑权限"这种设置</strong>，读者本来就拿不到可编辑的原文件
- **验证门槛**：国际版用邮箱验证、中文版用短信验证；MaiPDF 自己没有"设置口令"的功能，若 PDF 文件本身带密码，阅读器会照常要求输入
