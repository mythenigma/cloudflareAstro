...
动态水印的思路不是"阻止截图"，而是**让每一张截图都能对回到具体某一次访问**。MaiPDF 的水印会在每一页上盖上**本次打开的记录编号**——注意页面上只有这个编号，不会印出对方的 IP 或邮箱。无论对方截哪一页、怎么裁剪，截到的内容都带着编号；拿编号去查访问记录，IP、浏览器、时间就都出来了，开了验证的还能对到具体邮箱或手机号。

这一层的价值有两重：

- **事前威慑**：对方一打开就看到页面上盖着一串编号，明白这份文件是记名的、这次打开是被登记的，"随手截图转发"的动机被瞬间抑制。真正的顺手泄密大部分就这么被过滤掉。
...
- **对方需要完全离线阅读**（内网、没网环境）——链接型分享解决不了。走 **App DRM**（在 [drm.maipdf.com](https://drm.maipdf.com/) 生成加密 `.maipdf`，U 盘/局域网分发，用 App 打开），是另一条独立工作流。
